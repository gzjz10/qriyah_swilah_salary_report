...
export default function ReportsTab() {
  ...
  const deptData = useMemo(() => {
    ...
    employees.forEach((e) => { if(!m[e.dept]) m[e.dept]={emps:[],total:0,avg:0,max:0,min:Infinity}; m[e.dept].emps.push(e); m[e.dept].total+=e.salary; if(e.salary>m[e.dept].max) m[e.dept].max=e.salary; if(e.salary<m[e.dept].min) m[e.dept].min=e.salary; });
  ...
  return (
    <div>
      <div className="animate-fade-in-up" style={{ background:'var(--bg-glass)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', border:'1px solid var(--border2)', borderRadius:16, padding:'22px 24px', marginBottom:24, boxShadow:'var(--shadow)' }}>
        <div style={{ fontSize:14, fontWeight:700, marginBottom:16, color:'var(--text2)' }}>ملخص إجمالي الشركة</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12 }}>
          {[
            {lbl:'إجمالي الموظفين',val:employees.length.toString(),color:'var(--teal-light)'},
            {lbl:'إجمالي الرواتب',val:fmt(grandTotal),color:'var(--amber-light)'},
            ...
            <div key={lbl} style={{ background:'var(--bg-stat)', border:'1px solid var(--border2)', borderRadius:10, padding:'14px 16px', transition:'all var(--transition)' }}
              onMouseEnter={(e) => {e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.background='var(--bg-input)';}}
              onMouseLeave={(e) => {e.currentTarget.style.borderColor='var(--border2)';e.currentTarget.style.background='var(--bg-stat)';}}>
        ...
        {Object.entries(deptData).map(([dept,{emps,total,avg,max,min}],i) => {
          ...
          return (
            <div key={dept} className={`animate-fade-in-up stagger-${Math.min(i+1,5)}`}
              style={{ background:'var(--bg-glass)', backdropFilter:'blur(14px)', WebkitBackdropFilter:'blur(14px)', border:'1px solid var(--border2)', borderRadius:14, overflow:'hidden', transition:'all 220ms ease' }}
              ...
              <div style={{ padding:'14px 18px', background:`linear-gradient(135deg,${col}10,transparent)`, borderBottom:'1px solid var(--border2)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                ...
                <span style={{ fontSize:11, color:'var(--text3)', background:'var(--bg-inset)', padding:'3px 9px', borderRadius:7, border:'1px solid var(--border2)' }}>{emps.length} موظف</span>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:1, background:'var(--border2)' }}>
                {[{lbl:'إجمالي الرواتب',val:fmt(total)},{lbl:'متوسط الراتب',val:fmt(avg)},{lbl:'أعلى راتب',val:fmt(max)},{lbl:'أقل راتب',val:fmt(min)}].map(({lbl,val}) => (
                  <div key={lbl} style={{ background:'var(--bg-dark-block)', padding:'12px 14px' }}>
