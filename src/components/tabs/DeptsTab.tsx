...
export default function DeptsTab() {
  ...
  const deptData = useMemo(() => {
    ...
    employees.forEach((e) => {
      if (!m[e.dept]) m[e.dept] = { emps: [], total: 0, avg: 0 };
      m[e.dept].emps.push(e);
      m[e.dept].total += e.salary;
    });
    Object.values(m).forEach((d) => {
      d.emps.sort((a, b) => b.salary - a.salary);
      d.avg = Math.round(d.total / d.emps.length);
    });
    return m;
  }, [employees]);
      ...
      {Object.entries(deptData).map(([dept, { emps, total, avg }], i) => {
        ...
        return (
          ...
          >
            ...
            <div style={{
              padding: '16px 20px',
              background: `linear-gradient(135deg, ${col}12, transparent)`,
              borderBottom: '1px solid var(--border2)',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: col, textShadow: `0 0 14px ${col}44` }}>{dept}</div>
              <span style={{ fontSize: 11, color: 'var(--text3)', padding: '3px 9px', background: 'rgba(0,0,0,0.25)', borderRadius: 7, border: '1px solid var(--border2)' }}>
                {emps.length} موظف
              </span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'var(--border2)' }}>
              {[
                ...
                <div key={lbl} style={{ background: 'rgba(0,0,0,0.2)', padding: '12px 16px' }}>
            ...
            <div style={{ maxHeight: 230, overflowY: 'auto' }}>
              {emps.map((e) => (
                <div
                  ...
                  onMouseEnter={(el) => (el.currentTarget.style.background = `${col}0a`)}
            ...
            <div style={{ padding: '10px 14px', borderTop: '1px solid var(--border2)' }}>
              <button
                onClick={() => openModal({ type: 'dept', dept })}
