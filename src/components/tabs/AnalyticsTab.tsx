...
function ChartCard({ title, children }: { title:string; children:React.ReactNode }) {
  return (
    <div style={{ background:'var(--bg-glass)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', border:'1px solid var(--border2)', borderRadius:16, padding:'22px 24px', boxShadow:'var(--shadow)', transition:'border-color var(--transition)' }}
...
function HBar({ label, value, max, color, sub, delay=0 }: { label:string; value:number; max:number; color:string; sub?:string; delay?:number }) {
...
export default function AnalyticsTab() {
  ...
  const R=66,CX=76,CY=76,CIRC=2*Math.PI*R;
  let offset=0;
  const slices=deptTotals.map(([dept,val]) => { const pct=total>0?val/total:0; const dash=pct*CIRC; const gap=(1-pct)*CIRC; const dashOffset=-(offset*CIRC); offset+=pct; return{dept,val,dash,gap,dashOffset,color:DEPT_COLORS[dept]||'#F5A623'}; });
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:18 }}>
      <ChartCard title="توزيع الرواتب حسب القسم">
        <div style={{ display:'flex', alignItems:'center', gap:20 }}>
          <svg width={152} height={152} viewBox="0 0 152 152" style={{ flexShrink:0 }}>
            ...
            {slices.map((s) => (
              <circle key={s.dept} cx={CX} cy={CY} r={R} fill="none" stroke={s.color} strokeWidth={16}
            ...
            <text x={CX} y={CY+8} textAnchor="middle" fill="var(--amber-light)" fontSize={8} fontWeight="800" fontFamily="Cairo">{fmt(total)}</text>
          </svg>
      ...
      <ChartCard title="إجمالي الرواتب حسب الفرع">
        {branchTotals.map(([branch,val],i) => <HBar key={branch} label={branch} value={val} max={maxBranch} color={COLORS[i%COLORS.length]} delay={i*80} />)}
      </ChartCard>

      <ChartCard title="أعلى 10 رواتب">
        {top10.map((e,i) => <HBar key={e.id} label={`${i+1}. ${e.name.split(' ').slice(0,2).join(' ')}`} value={e.salary} max={maxTop} color={COLORS[i%COLORS.length]} delay={i*60} />)}
      </ChartCard>

      <ChartCard title="توزيع الرواتب بالفئات">
        {ranges.map((r,i) => <HBar key={r.lbl} label={r.lbl} value={r.count} max={maxRange} color={COLORS[i%COLORS.length]} sub={`${r.count} موظف`} delay={i*80} />)}
      </ChartCard>
    </div>
  );
}
