...
const BRANCH_ICON: Record<string, string> = {
  قرطبة: '🏙', 'مستودع الشفا': '🏭', 'مستودع قوين': '📦',
};

const DEPT_STAT_COLORS = [
  { accent: '#E8B86D', glow: 'rgba(232,184,109,0.15)' },
...
export default function SalariesTab() {
  ...
  const deptStats = useMemo(() => {
    ...
    employees.forEach((e) => {
      if (!m[e.dept]) m[e.dept] = { count: 0, total: 0 };
      m[e.dept].count++;
  ...
  const filtered = useMemo(() => {
    ...
    rows = rows.slice().sort((a, b) => sortDesc ? b.salary - a.salary : a.salary - b.salary);
    return rows;
    ...
    <div>
        ...
        {Object.entries(deptStats).map(([dept, s], i) => {
          ...
          return (
            <button
              ...
              style={{
                background: active ? `rgba(${hexToRgb(accent)},0.12)` : 'rgba(255,255,255,0.03)',
                backdropFilter: 'blur(12px)',
                WebkitBackdropFilter: 'blur(12px)',
                border: `1px solid ${active ? accent + '55' : 'var(--border2)'}`,
                borderRadius: 14,
                padding: '18px 20px',
                cursor: 'pointer',
                textAlign: 'right',
            ...
            >
              ...
              {active && (
                <div style={{ position: 'absolute', top: 10, left: 10, width: 6, height: 6, borderRadius: '50%', background: accent, boxShadow: `0 0 8px ${accent}` }} />
              )}
            </button>
          );
        })}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }} data-no-print>
        ...
        <button
          ...
          style={{
            background: 'rgba(255,255,255,0.04)',
            border: `1px solid ${sortDesc ? 'var(--gold)' : 'var(--border2)'}`,
            borderRadius: 10,
            padding: '9px 16px',
            color: sortDesc ? 'var(--gold-light)' : 'var(--text2)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 22px', borderBottom: '1px solid var(--border2)' }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700 }}>قائمة الموظفين</div>
            <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
              {filtered.length} موظف&nbsp;&nbsp;·&nbsp;&nbsp;إجمالي: <span style={{ color: 'var(--gold-light)', fontWeight: 700 }}>{fmt(filtered.reduce((s, e) => s + e.salary, 0))}</span>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7}>
                    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text3)' }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600 }}>لا توجد نتائج مطابقة</div>
                    </div>
                  </td>
                </tr>
              ) : (
                filtered.map((e, idx) => {
                  ...
                  const acc  = DEPT_ACCENT[e.dept] || { bg: 'rgba(200,150,62,0.1)', text: 'var(--gold-light)', border: 'rgba(200,150,62,0.3)' };

                  let rankBg = 'var(--dark4)'; let rankColor = 'var(--text3)'; let rankShadow = 'none';
                  ...
                  return (
                    <tr
                      key={e.id}
                      className={`animate-fade-in-up stagger-${Math.min((idx % 8) + 1, 5)}`}
                      ...
                      onMouseEnter={(el) => (el.currentTarget.style.background = 'rgba(200,150,62,0.05)')}
                    ...
                    >
                      <td style={{ padding: '11px 18px' }}>
                        <div style={{
                          ...
                          boxShadow: rankShadow, flexShrink: 0,
                        }}>{rank}</div>
                      </td>
                      <td style={{ padding: '11px 18px', fontWeight: 600, fontSize: 13 }}>{e.name}</td>
                      ...
                      <td style={{ padding: '11px 18px' }}>
                        <span style={{
                          ...
                          background: acc.bg, color: acc.text,
                          border: `1px solid ${acc.border}`,
                          letterSpacing: '0.01em',
                        }}>{e.dept}</span>
                      </td>
                      <td style={{ padding: '11px 18px', fontSize: 12, color: 'var(--text3)' }}>
                        {BRANCH_ICON[e.branch] || '🏢'} {e.branch}
                      </td>
                      <td style={{ padding: '11px 18px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 800, fontSize: 13, color: 'var(--gold-light)' }}>
                          {fmt(e.salary)}
                          <div className="salary-bar">
                            <div className="salary-fill bar-animate" style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: '11px 18px' }}>
                        <div style={{ display: 'flex', gap: 6 }} onClick={(ev) => ev.stopPropagation()}>
                          <button
                            aria-label="تعديل"
                            onClick={() => openModal({ type: 'edit', employee: e })}
                            style={{ background: 'rgba(200,150,62,0.1)', border: '1px solid rgba(200,150,62,0.25)', color: 'var(--gold-light)', padding: '6px 10px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all var(--transition)' }}
                          ...
                          <button
                            aria-label="حذف"
                            onClick={() => openModal({ type: 'delete', employee: e })}
                            style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: 'var(--red)', padding: '6px 10px', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all var(--transition)' }}
